...
private:
  bool initialize_vulkan_resources() {
    ...
    mem_alloc.memoryTypeIndex = wf_vk_utils::find_memory_type(
  ...
  wf::post_hook_t render_hook = [=](wf::auxilliary_buffer_t &source,
    ...
    int32_t base_x = 200 + (int32_t)(100.0f * std::cos(elapsed * 2.0f));
    int32_t base_y = 200 + (int32_t)(100.0f * std::sin(elapsed * 2.0f));
    int32_t dynamic_size = 100 + (int32_t)(50.0f * std::sin(elapsed * 4.0f));
    ...
    wf_vk_utils::transition_layout(ctx->cmd, ctx->scratch_image,
    ...
    wf_vk_utils::transition_layout(ctx->cmd, src_attribs.image,
                                   VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
                                   VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL);
    wf_vk_utils::transition_layout(ctx->cmd, dest_attribs.image,
    ...
    rect_blit.dstOffsets[0] = VkOffset3D{base_x, base_y, 0};
    rect_blit.dstOffsets[1] =
        VkOffset3D{base_x + dynamic_size, base_y + dynamic_size, 1};
